...
def show_images(batch: th.Tensor):
    ...
    Image.fromarray(reshaped.numpy()).save('inpainted_image.png')
...
def read_mask(path: str, size: int = 256) -> Tuple[th.Tensor, th.Tensor]:
    ...
    pil_img = pil_img.resize((size, size), resample=Image.BICUBIC)
    img = np.array(pil_img)
    img_ = th.tensor((img > 0)).float().unsqueeze(0).unsqueeze(0)
    return img_

# Sampling parameters
prompt = "a photo of a rail"
batch_size = 1
guidance_scale = 5.0
...
source_image_256 = read_image('src_img/train.jpg', size=256)
source_image_64 = read_image('src_img/train.jpg', size=64)
...
source_mask_256 = read_mask('src_mask/mask_01.png' , size=256) 
source_mask_64 = read_mask('src_mask/mask_01.png' , size=64)
